            ...
            slip_file: {
                ...
                compute(slip_file) {
                    ...
                    if (slip_file.slip_file != null) {
                        slip_file_1 =
                            process.env.PATH_UPLOAD + slip_file.slip_file;
                    }
                    return slip_file_1;
                },
            },
...
const onCheckBookingDateBeforeApprove = async (id) => {
    try {
        ...
        if (
            ...
            check_period_time_available[item1.period_time - 1].available ==
                false ||
            (item1.period_time == 3 &&
...
const methods = {
    ...
    async onCreate(req, res) {
        try {
            ...
            for (let i = 0; i < req.body.equipment_booking_method.length; i++) {
                let data_method = {
                    ...
                    quantity: Number(
                        req.body.equipment_booking_method[i].quantity
                    ),
                    price: Number(
                        req.body.equipment_booking_method[i].total_price
                    ),
                    created_by: "arnonr",
                    updated_by: "arnonr",
                };
            ...
            await transporter.sendMail({
                from: "ศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยี คณะวิทยาศาสตร์ประยุกต์", // อีเมลผู้ส่ง
                to: item.email, // อีเมลผู้รับ สามารถกำหนดได้มากกว่า 1 อีเมล โดยขั้นด้วย ,(Comma)
                subject:
                    "รายการเช็คค่าบริการ และประมาณค่าใช้จ่ายผ่านระบบอัตโนมัติจากศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยี", // หัวข้ออีเมล
                html: `<div
         style="
       ...
       >
         ...
         <div style="font-size: 20px; margin-bottom: 22px; max-width: 800px">
           ศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยีขอขอบพระคุณเป็นอย่างสูง
    ...
    async onUpdate(req, res) {
        try {
                ...
                data: {
                    ...
                    member_status:
                        req.body.member_status != null
                            ...
                            : undefined,
                    example:
                        req.body.example != null ? req.body.example : undefined,
                    prefix:
                    ...
                    invoice_address:
                        req.body.invoice_address != null
                            ...
                            : undefined,
                    tax_id:
                        req.body.tax_id != null ? req.body.tax_id : undefined,
            ...
            if (req.body.equipment_booking_method) {
                for (
                    let i = 0;
                    i < req.body.equipment_booking_method.length;
                    i++
                ) {
                    let data_method = {
                        quantity: Number(
    ...
    async onApprove(req, res) {
        try {
            ...
            //     } else {
            //         text["subject"] = "การจองของท่านได้รับการยกเลิก";
            //     }
            ...
            //         html:
            //             "<b>ศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยี คณะวิทยาศาสตร์ประยุกต์</b><br> ดูรายละเอียดได้ที่ : <a href='" +
            //             process.env.PATH_CLIENT +
            //             "booking" +
            //             "'>คลิก</a>", // html body
            //     });
            ...
            if (item.status_id == 2) {
                ...
                text["subject"] =
                    "กรุณาโอนเงินเพื่อชำระค่าบริการ/ทดสอบของศูนย์ SITT";
                ...
                await transporter.sendMail({
                    ...
                    html:
                        ...
                        "<br>ยอดทำรายการจำนวน " +
                        item.price +
                        " บาท กรุณาโอนเงินเพื่อชำระค่าบริการ/ทดสอบของศูนย์ SITT</b><br> ดูรายละเอียดได้ที่ : <a href='" +
                        process.env.PATH_CLIENT +
                        "booking/" +
                        item.id +
                        "'>คลิก</a>", // html body
            ...
            if (item.status_id == 3) {
                ...
                await transporter.sendMail({
                    ...
                    html:
                        "<b>เลขคำสั่งจอง: " +
                        item.code +
                        "<br>การจองของท่านได้รับการยกเลิก เนื่องจาก  " +
                        item.reject_comment +
                        "</b><br> ดูรายละเอียดได้ที่ : <a href='" +
                        process.env.PATH_CLIENT +
                        "booking/" +
                        item.id +
                        "'>คลิก</a>", // html body
            ...
            if (item.status_id == 6) {
                ...
                await transporter.sendMail({
                    ...
                    html:
                        "<b>เลขคำสั่งจอง: " +
                        item.code +
                        "<br>การชำระเงินสำเร็จ กรุณามาตาม วัน/เวลาที่นัดหมาย" +
                        "<br>" +
                        "ใบเสร็จรับเงิน : " +
                        "<a href='" +
                        item.invoice_file +
            ...
            if (item.status_id == 7) {
                ...
                let text = {};
                text["subject"] = "ขอบคุณที่ใช้บริการของเรา";
    ...
    //             if (item.status_id == 8) {
    //                 text["subject"] =
    //                     "การชำระเงินของท่านได้รับการตรวจสอบเรียบร้อย";
    //             } else {
    //                 text["subject"] =
    //                     "การชำระเงินของท่านได้รับการตรวจสอบเรียบร้อย";
    //             }
        ...
        try {
                ...
                data: {
                    ...
                    slip_date:
                        req.body.slip_date != null
                            ...
                            : undefined,
                    slip_at:
                        req.body.slip_at != null
                            ? new Date(req.body.slip_at)
                            : undefined,
                    status_id: 5,
                    updated_by: "arnonr",
                },
            ...
            if (email1.length > 0) {
                await transporter.sendMail({
                    ...
                    html:
                        "<b>ศูนย์เครื่องมือวิทยาศาสตร์และถ่ายทอดเทคโนโลยี คณะวิทยาศาสตร์ประยุกต์</b><br> โปรดตรวจสอบหลักฐานการชำระเงินที่ : <a href='" +
                        process.env.PATH_CLIENT +
                        "admin/booking" +
                        "'>คลิก</a>", // html body
                });
        ...
        try {
            ...
            const item = await prisma.equipment_booking.findMany({
                where: {
                    ...
                    booking_date: {
                        gte: new Date(
                            req.query.booking_date + " 00:00:00"
                        ).toISOString(),
                        lte: new Date(
                            req.query.booking_date + " 23:59:00"
                        ).toISOString(),
                    },
                    status_id: 2,
                },
